...
def PuLP_Solver(faculties, athletes, ranking, available_pullovers, pullovers_for_referees_and_teachers, pullovers_for_aaac, preferences = {}):
    ...
    faculties_under_10 = {i: athletes[i] for i in faculties if i in athletes and athletes[i] < 10}
    ...
    x = {i: pulp.LpVariable(f"x_{i}", lowBound=0, cat="Integer") for i in remaining_faculties}
    y = {(i, j): pulp.LpVariable(f"y_{i}_{j}", cat="Binary") for i in remaining_faculties for j in colors}
    proportions = {i: pulp.LpVariable(f"proportion_{i}", lowBound=0) for i in remaining_faculties}
    difference = {i: pulp.LpVariable(f"difference_{i}", lowBound=0) for i in remaining_faculties}
    z = {(i, j): pulp.LpVariable(f"z_{i}_{j}", lowBound=0, cat="Continuous") for i in remaining_faculties for j in colors}
    ...
    for j in colors:
        prob += pulp.lpSum(z[i, j] for i in remaining_faculties) <= available_pullovers[j]
